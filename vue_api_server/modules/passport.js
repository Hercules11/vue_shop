...
const LocalStrategy = require('passport-local').Strategy // default name: local
const Strategy = require('passport-http-bearer').Strategy // default name: bearer
...
module.exports.setup = function (app, loginFunc, callback) {
  ...
  passport.use(new Strategy(
    function (token, done) {
      jwt.verify(token, jwtConfig.get('secretKey'), function (err, decode) {
...
module.exports.login = function (req, res, next) {
  passport.authenticate('local', function (err, user, info) {
    ...
    // 获取角色信息
    var token = jwt.sign({ uid: user.id, rid: user.rid }, jwtConfig.get('secretKey'), { expiresIn: jwtConfig.get('expiresIn') })
